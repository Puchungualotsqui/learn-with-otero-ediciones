...
import (
	...
	"frontend/templates/components/pdfViewer"
	"net/http"
	"strconv"
)
...
func HandleAssetsDefault(store *database.Store, w http.ResponseWriter, r *http.Request, classId int) {
	...
	assets = database.FixNames(assets)

	render.RenderWithLayout(w, r, panelsContent.PanelsContent(pdfViewer.PDFViewer("", assets)), body.Home)
}
